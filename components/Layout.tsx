      ...
      <aside className="fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 text-white flex flex-col shadow-2xl transition-transform duration-300 ease-in-out desktop-sidebar min-[993px]:relative min-[993px]:translate-x-0 no-print">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
          {navItems.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => handleNavClick(item.id)} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200 group relative nav-item-btn ${isActive ? 'bg-blue-600/10 text-blue-400 shadow-sm border border-blue-600/20' : 'text-slate-500 hover:bg-slate-800/40 hover:text-slate-300'}`}>
